...
export default function AuthLayout({
    ...
    const heroDescription =
        description ||
        'Manage shifts, authorize requests, and sync your people from anywhere in the world.';

    const isRegisterView = context === 'register';
    const ctaHref = isRegisterView ? register().url : login().url;
    ...
    const highlights = [
        {
            ...
            description:
                'Shift approvals, coverage rules, and payroll roles stay mirrored in real time.',
        },
        {
            title: 'Coverage intelligence',
            description:
                'Cross-location dashboards surface open needs before they become overtime emergencies.',
        },
        {
            title: 'Scheduling autopilot',
            description:
                'Managers publish templates; teams confirm on mobile and Horizon handles the API sync.',
        },
    ];
    ...
    return (
        <div className="relative min-h-screen overflow-hidden bg-[#030712] text-white">
            <div className="pointer-events-none absolute inset-0">
                <div className="animate-float absolute top-12 -left-32 h-80 w-80 rounded-full bg-(--accent-blue)/25 blur-[180px]" />
                <div className="animate-float absolute -right-24 bottom-0 h-125 w-125 rounded-full bg-(--accent-purple)/25 blur-[220px] delay-200" />
            ...
            <div className="relative z-10 flex min-h-screen items-center justify-center px-2 py-12 lg:px-10">
                <main className="relative w-full max-w-6xl overflow-hidden rounded-[48px] border border-white/10 bg-white/5 shadow-[0_30px_120px_rgba(2,6,23,0.75)] backdrop-blur-2xl">
                    ...
                    <div className="grid gap-10 px-2 py-2 lg:grid-cols-[1.05fr_0.95fr] lg:px-14 lg:py-16">
                        <section className="animate-fadeInLeft flex flex-col gap-10 rounded-4xl border border-white/10 bg-white/5 p-3 shadow-inner shadow-white/5 backdrop-blur-2xl">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 px-5 py-4 text-sm text-white/80">
                                ...
                                <Link
                                    href={ctaHref}
                                    className="rounded-full bg-white px-4 py-1.5 text-xs font-semibold tracking-[0.3em] text-primary-700 uppercase"
                                >
                                    {ctaLabel}
                                </Link>
                            </div>
                        ...
                        <section className="animate-fadeInUp relative">
                            ...
                            <div className="animate-fadeInDown pointer-events-none absolute -top-7 right-0 hidden rounded-2xl border border-white/10 bg-primary-500/50 px-5 py-4 text-sm font-semibold text-white shadow-2xl backdrop-blur delay-200 lg:block">
